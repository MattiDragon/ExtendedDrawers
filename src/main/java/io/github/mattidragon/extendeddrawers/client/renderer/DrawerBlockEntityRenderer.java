...
public class DrawerBlockEntityRenderer extends AbstractDrawerBlockEntityRenderer<DrawerBlockEntity> {
    ...
    public void render(DrawerBlockEntity drawer, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        switch (slots) {
            case 1 -> renderSlot(drawer.storages[0], false, light, matrices, vertexConsumers, (int) drawer.getPos().asLong(), overlay, blockPos);
            case 2 -> {
                matrices.translate(-0.25, 0, 0);
                renderSlot(drawer.storages[0], true, light, matrices, vertexConsumers, (int) drawer.getPos().asLong(), overlay, blockPos);
                matrices.translate(0.5, 0, 0);
                renderSlot(drawer.storages[1], true, light, matrices, vertexConsumers, (int) drawer.getPos().asLong(), overlay, blockPos);
            }
            case 4 -> {
                ...
                renderSlot(drawer.storages[2], true, light, matrices, vertexConsumers, (int) drawer.getPos().asLong(), overlay, blockPos);
                matrices.translate(0.5, 0, 0);
                renderSlot(drawer.storages[3], true, light, matrices, vertexConsumers, (int) drawer.getPos().asLong(), overlay, blockPos);
            }
    ...
    private void renderSlot(DrawerSlot storage, boolean small, int light, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int seed, int overlay, BlockPos pos) {
        ...
        renderSlot(storage.isHidden() ? ItemVariant.blank() : storage.getItem(), ((storage.getAmount() == 0) || ClientConfig.HANDLE.get().displayEmptyCount()) ? null : storage.getAmount(), small, icons, matrices, vertexConsumers, light, overlay, seed, pos);
    }
}
